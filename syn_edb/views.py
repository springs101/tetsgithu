...
def getData_multiprocess(processtype, benginday, endday):  # 多进程调用接口
    ...
    minuteProcesstype = [
        'getData_minute_orderdate',
        'getData_minute_paydate',
        'getData_minute_deliverydate',
        'getData_minute_reviewtime',
        'getData_minute_cancletime',
        'getData_minute_complatetime',
        'getData_minute_inspecttime',
        'getData_minute_weighttime']  # ['订货日期','付款日期','发货日期','审单时间','取消时间','完成时间','验货日期','称重时间']

    obj = models.AJobLog.objects.filter(
        status=1,
        jobname__startswith='getData_minute').aggregate(
        Max('endTime'))
    timeMax = datetime.datetime.strftime(
        obj["endTime__max"], '%Y-%m-%d %H:%M:%S')

    poolhandle = Pool(8)
    if processtype == 'getData_minute':
        timespliceend = time.strftime(
            '%Y-%m-%d %H:%M:%S',
            time.localtime(
                time.time()))
        ...
        for tp in minuteProcesstype:
            for shopIdarray in listshopId:
                # print("开辟分钟进程")
                poolhandle.apply_async(
                    getData_sub,
                    kwds={
                        'shopid': shopIdarray,
                        'begindate': str(timesplicestart),
                        'enddate': str(timespliceend),
                        'processtype': str(tp)})
    else:
        for shopIdarray in listshopId:
            ...
            poolhandle.apply_async(
                ...
                kwds={
                    'shopid': shopIdarray,
                    'begindate': str(benginday),
                    'enddate': str(endday),
                    'processtype': str(processtype)})
...
def getData_onemonth():
    ...
    getData_multiprocess(
        'getData_month',
        up_first_day.strftime("%Y-%m-%d"),
        up_last.strftime("%Y-%m-%d"))
    return
...
def getData_minute():
    obj = models.AJobLog.objects.filter(
        status=0, jobname__startswith='getData_minute')
    if obj.exists():
...
def bakGetDataLog(request):
    ...
    logger.info('查询开始')
    objs = models.AJobLog.objects.filter(
        status=1,
        jobname__startswith='getData_minute',
        createTime__lte=sevendaybeforeDay)
    outputdic = []
    for obj1 in objs:
    ...
    try:
        ...
        models.AJobLog.objects.filter(
            status=1,
            jobname__startswith='getData_minute',
            createTime__lte=sevendaybeforeDay).delete()
...
def getData_remedy(request):
    ...
    if begindate_error is None or begindate_error == '':
        ...
        if isVaildDate(begindate_error) == False:
            ...
            return getresponse(err_msg)
    if enddate_error is None or enddate_error == '':
    ...
    if shopId_error is None or shopId_error == '':
    ...
    if error_flag is None or error_flag == '':
...
def getData_sub(**kwds):  # 具体计算
    ...
    minuteProcesstype = {
        'getData_minute_orderdate': '订货日期',
        'getData_minute_paydate': '发货日期',
        'getData_minute_deliverydate': '审单时间',
        'getData_minute_reviewtime': '取消时间',
        'getData_minute_cancletime': '取消时间',
        'getData_minute_complatetime': '完成时间',
        'getData_minute_inspecttime': '验货日期',
        'getData_minute_weighttime': '称重时间'}
    ...
    for id in listarray:
        for var in daylist:
            if processtype in minuteProcesstype.keys():
                ...
                begindate = str(kwds['begindate'])
                enddate = str(kwds['enddate'])
                totalnum1 = getTotalNum(
                    begindate, enddate, str(id), minuteProcesstype[processtype])
            else:
                logger.info("No")
            ...
            if totalnum1 == '0':
                ...
                logger.info(
                    ...
                    ',' +
                    str(id))
                handleConnPool1.updateLog(
                    processtype, begindate, enddate, str(id))
                continue
...
def getbeforfiveminute(current):
    ...
    beforeTime = (ctime - datetime.timedelta(minutes=10)
...
def getformEDB(bgdate, enddate, shopId, pageNum, pagesize, mo,
               isMultiProcess=0, handleConnPool=None, data_type='订货日期'):
    importlib.reload(sys)
